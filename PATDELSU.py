...
def Search_button():
    ...
    if (len(p)==0):
        ...
        for i in t:
            ...
            l3=tkinter.Label(rootS,text="PATIENT GENDER",fg='blue')
...
def up1():
    ...
    if len(p) != 0:
        conn.execute('UPDATE PATIENT SET NAME=?,GENDER=?,DOB=?,BLOOD_GROUP=?,ADDRESS=?,CONSULT_TEAM=?,EMAIL=? where PATIENT_ID=?', ( u2, u3, u4, u5, u10, u9, u8,u1,))
...
def P_UPDATE():
    ...
    gender = tkinter.Label(rootU, text="GENDER")
    pat_gender = tkinter.Entry(rootU)
